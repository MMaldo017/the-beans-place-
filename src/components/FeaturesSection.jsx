...
//all 3 rows hold the same 15 bags in different orders, so the rows don't look like copies of eachother as they slide past
...
//draws on horizontal row. local to this file since nothing else uses it.
...
function ImageRow({images,offset = 0}){
    ...
    return(
        ...
        //translated slides the row and runs on the gpu, which keeps it smooth
        <div className ="carousel-row" style={{transform: `translate3d(${offset}px, 0, 0)`}}>
            {doubled.map((src,index) => (
                ...
                <div className ="carousel-card" key={`${index}`}>
                    ...
                    //% modulo wraps the count, so the duplicated half reuses 1-15 instead of running on to 30
...
export default function FeaturesSection(){
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            //p ("how far scrolled") x range = how far we slide

            setOffsets([
                -p * range, // row 1 is left
    ...
    return (
        ...
        <section className ="carousel-gallery-section" ref={sectionRef}>
            ...
            <div className="carousel-gallery-container">
                {/* each row gets its own image list and its own live offset */}
                <ImageRow images={row1} offset={offsets[0]} />
                <ImageRow images={row2} offset={offsets[1]} />
                <ImageRow images={row3} offset={offsets[2]} />
            </div>

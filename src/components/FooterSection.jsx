...
//React Hooks: useEffect runs code after a render, useRef sotres a value / DOM reference
//that survives re-renders without causing ones
import {useEffect, useRef} from "react";
//Leaflet: library for interactive maps
import { Layer } from "leaflet";
//Leaflete's required stylesheet
...
const navigation = {
    //links for the shop column each has a display texty and url
    shop: [
...
function LocationMap(){
     ...
     //mapref --> points to the div the map draws into
    const mapRef = useRef(null);
    //mapInstance -> stores the created map so we dont build it twice
    const mapInstance = useRef(null);
    //runs once after the component mounts ( emptin [] at the end = run once)
    useEffect(() => {
        ...
        L.marker([lat, long], { icon })
            ...
            //clean up: remove the map if the component unmounts to prevent links
            return() => {
                map.remove();
    ...
    return (
       //the empty div leflet fills with the map
    ...
    return(
        <footer className="footer">
            <div className = "mx-auto max-w-7x1 px-6 pb-8 pt-16 sm:pt-24 lg:pt-32 2x1:max-w-400">
                {/* Map (wrapped in scroll reveal so it fades up into view on scoll) */}
                <ScrollReveal animation ="fadeUp">
                    <LocationMap />
                </ScrollReveal>

                {/* footer columns w columns on small screens, 4 on md+ */}
                <div className="mt-16 grid grid-col-2 gap-8 md:grid-cols-4">
                    {/* first column: logo, description, social icone */}
                    ...
                    {/* shop column - built by looping navigation. shop */}
                ...
                {/* divider line about the copyright row */}

...
// card is the default export, the brached names are extra imports that are from that file
    ...
    return (
        <div className="about-content">
            ...
            <div className="about-story-grid">
                {/* fadeleft slides the text in left */}
                <ScrollReveal animation="fadeLeft" className="about-story-text">
                    ...
                    <p className="about-body lead--light">
                        Every week, our roastmaster cups dozens of samples to select only the lots that meet our standard.  Whether you prefer a bright, fruity Ethiopian Yirgacheffe or a deep, chocolately Colombian Supremo, we roast each batch to order so it arrives at peak freshness. 
                    </p>
                </ScrollReveal>

                    {/* photos slide in from the right, just after the text */}
                    <ScrollReveal animation="fadeRight" delay={.2} className="about-story-images">
                        {/* two images stack so the second overlaps a corner  */}
                        <div className="about-image-stack">
                            <img
                            src={coffeePlants}
                            alt="Coffee plants on farm"
            ...
            {/* valuecards: each starts 0.2s after the one before */}
            <StaggerContainer staggerDelay={.15} className="about-cards-grid">
                {values.map((v) => (
                    //key must be unique - react uses it to track list items
                    <StaggerItem key={v.title} animation="fadeUp">
                        {/* bare glass means glass=true frosted glass look */}
                        <Card glass className="about-value-card">
                            <CardHeader>
